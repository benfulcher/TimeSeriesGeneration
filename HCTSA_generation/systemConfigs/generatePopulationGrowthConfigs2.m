function TSconfigs = generatePopulationGrowthConfigs2()
% parameter values to generate the TSconfig for
rVar =      [0.1, 3];        rFix = 0.5;
KVar =      [1.5, 5];        KFix = 1;
betaVar =   [1, 50];    betaFix = 5;
...
for paramsInd = 1:length(paramsCell)
    TSconfigs = horzcat(TSconfigs, generatePopulationGrowthConfig(paramsCell{paramsInd}, Ns(paramsInd)));
end

end

function config = generatePopulationGrowthConfig(params, N)
